-- PixelGuess online lobby schema.
...
create table public.rooms (
  ...
  constraint rooms_status_values check (status in ('waiting', 'playing', 'finished')),
  constraint rooms_max_players_range check (max_players between 2 and 6)
);
...
create table public.room_players (
  ...
  constraint room_players_name_length check (
    char_length(display_name) between 2 and 16
    and display_name = btrim(display_name)
  ),
  constraint room_players_score_nonnegative check (score >= 0),
  constraint room_players_room_user_unique unique (room_id, user_id)
);

create unique index room_players_room_name_unique
  on public.room_players (room_id, lower(display_name));

create index room_players_user_id_idx
  on public.room_players (user_id);

alter table public.rooms enable row level security;
alter table public.room_players enable row level security;

create or replace function private.is_room_member(check_room_id bigint)
returns boolean
language sql
stable
security definer
...
revoke all on function private.is_room_member(bigint) from public, anon;
grant execute on function private.is_room_member(bigint) to authenticated;
